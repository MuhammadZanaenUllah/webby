...
export function AppSidebar({ user }: AppSidebarProps) {
    ...
    return (
        <Sidebar variant="floating" collapsible="icon" className="group/sidebar border-none bg-transparent transition-all duration-300">
            ...
            <SidebarHeader className="h-[80px] px-6 group-data-[collapsible=icon]:px-0 flex-row items-center border-b border-primary/5 mb-2 relative z-10 transition-all duration-300">
                <Link href="/create" className="flex items-center w-full justify-center transition-transform hover:scale-[1.02]">
                    <ApplicationLogo showText={!isCollapsed} size={!isCollapsed ? "lg" : "md"} />
                </Link>
            </SidebarHeader>
            ...
            <SidebarContent className="!overflow-hidden flex-1 relative z-10 px-2">
                <div ref={scrollAreaRef} className="h-full">
                    <ScrollArea className="h-full [&_[data-slot=scroll-area-scrollbar]]:opacity-0 [&_[data-slot=scroll-area-scrollbar]]:transition-opacity group-hover/sidebar:[&_[data-slot=scroll-area-scrollbar]]:opacity-100" type="always">
                                    ...
                                    <SidebarMenuItem>
                                        ...
                                        >
                                            <Link href="/create" className="flex items-center justify-center w-full h-full">
                                                ...
                                                <span className="truncate transition-all duration-300 group-data-[collapsible=icon]:hidden opacity-100">{t("Launch AI")}</span>
                                            </Link>
                                        </SidebarMenuButton>
                                    </SidebarMenuItem>
                                </SidebarMenu>
                        ...
                        <Collapsible open={workspaceOpen} onOpenChange={setWorkspaceOpen} className="group/collapsible">
                            <SidebarGroup>
                                <CollapsibleTrigger asChild>
                                    <SidebarGroupLabel className="cursor-pointer hover:bg-primary/5 rounded-xl px-4 py-2 flex items-center justify-between text-[10px] uppercase font-black tracking-widest text-muted-foreground/60">
                                        <span className="group-data-[collapsible=icon]:hidden">{t("Workspace")}</span>
                                        <ChevronDown className="h-3 w-3 transition-transform group-data-[state=closed]/collapsible:rotate-[-90deg] group-data-[collapsible=icon]:hidden" />
                                        <div className="hidden group-data-[collapsible=icon]:block w-full h-[1px] bg-primary/10" />
                                    </SidebarGroupLabel>
                                </CollapsibleTrigger>
                                        ...
                                        <SidebarMenu className="gap-1">
                                            {recentProjects && recentProjects.length > 0 && (
                                                <Collapsible open={recentOpen} onOpenChange={setRecentOpen}>
                                                    <SidebarMenuItem>
                                                        <CollapsibleTrigger asChild>
                                                            <SidebarMenuButton className="group/recent h-10 px-4 rounded-xl hover:bg-primary/5 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0">
                                                                ...
                                                                <span className="font-bold text-sm group-data-[collapsible=icon]:hidden">{t("Recent")}</span>
                                                            </SidebarMenuButton>
                                                        </CollapsibleTrigger>
                                                        <CollapsibleContent>
                                                            <div className="ml-6 mt-1 mb-2 space-y-1 border-s border-primary/10 group-data-[collapsible=icon]:hidden">
                                                                {recentProjects.map((project) => {
                                                                    const displayName = project.name.length > 25 ? project.name.slice(0, 25) + "..." : project.name;
                                                                    return (
                                                                        <div key={project.id} className="px-2">
                                                                            <Link href={`/project/${project.id}`} className="flex items-center h-8 px-3 text-xs font-medium text-muted-foreground/70 hover:text-primary hover:bg-primary/5 rounded-lg transition-all" title={project.name}>
                                                ...
                                                <SidebarMenuItem key={item.titleKey}>
                                                    <SidebarMenuButton
                                                        ...
                                                        className={`h-11 px-4 transition-all rounded-xl group group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:!p-0 ${
                                                            isActive(item.href) ? "bg-primary/10 text-primary font-black shadow-[inset_0_0_20px_rgba(var(--primary-rgb),0.1)]" : "hover:bg-primary/5 text-muted-foreground/80 hover:text-foreground"
                                                        }`}
                                                    >
                                                        <Link href={item.href} className="flex items-center justify-center w-full h-full">
                                                            <item.icon className={`h-5 w-5 shrink-0 ${isActive(item.href) ? "text-primary" : "text-muted-foreground/50 group-hover:text-primary transition-colors"}`} />
                                                            <span className="ms-3 font-bold tracking-tight truncate transition-all duration-300 group-data-[collapsible=icon]:hidden opacity-100">{t(item.titleKey)}</span>
                                                        </Link>
                                                    </SidebarMenuButton>
                                                </SidebarMenuItem>
                                            ))}
                                                    ...
                                                    <SidebarMenuItem key={item.titleKey}>
                                                        <SidebarMenuButton
                                                            ...
                                                            className={`h-9 px-4 transition-all rounded-lg group group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:!p-0 ${
                                                                isActive(item.href) ? "bg-primary/10 text-primary font-bold" : "hover:bg-primary/5 text-muted-foreground/70 hover:text-foreground"
                                                            }`}
                                                        >
                                                            <Link href={item.href} className="flex items-center justify-center w-full h-full">
                                                                <item.icon className={`h-4 w-4 shrink-0 ${isActive(item.href) ? "text-primary" : "text-muted-foreground/40"}`} />
                                                                <span className="ms-3 text-xs font-bold truncate transition-all duration-300 group-data-[collapsible=icon]:hidden opacity-100">{t(item.titleKey)}</span>
                                                            </Link>
                                                        </SidebarMenuButton>
                                                    </SidebarMenuItem>
                                                ))}
